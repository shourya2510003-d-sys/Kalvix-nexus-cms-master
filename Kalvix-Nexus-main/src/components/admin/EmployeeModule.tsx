...
import { ref, get, set, push, remove, onValue, runTransaction } from 'firebase/database';
...
import {
  ...
  ChevronLeft, QrCode, FileText, Loader2, AlertTriangle
} from 'lucide-react';
...
type View = 'list' | 'add' | 'edit' | 'profile';
...
function generateEmployeeId(count: number): string {
  return `KNX-EMP-${String(count + 1).padStart(4, '0')}`;
}
...
async function generateCertId(): Promise<string> {
  ...
  return `KNX-${year}-${String(newCount).padStart(6, '0')}`;
}
...
async function generateCertificatePDF(
  ...
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', 20, 17, 12, 12);
  }
  ...
  doc.text('KALVIX NEXUS', W / 2, 25, { align: 'center' });

  doc.setFontSize(7);
  doc.setTextColor(180, 160, 80);
  doc.text('WHERE VISION MEETS TECHNOLOGY', W / 2, 30, { align: 'center' });

  // Certificate title
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(22);
  doc.setTextColor(10, 10, 10);
  doc.text('CERTIFICATE OF', W / 2, 55, { align: 'center' });
  doc.setFontSize(28);
  doc.setTextColor(212, 175, 55);
  doc.text(certType.toUpperCase(), W / 2, 67, { align: 'center' });

  // Divider line
  doc.setDrawColor(212, 175, 55);
  doc.setLineWidth(0.8);
  doc.line(40, 73, W - 40, 73);

  // Body text
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(11);
  doc.setTextColor(60, 60, 60);
  doc.text('This is to proudly certify that', W / 2, 84, { align: 'center' });

  // Employee name
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(20);
  doc.setTextColor(10, 10, 10);
  doc.text(employee.name.toUpperCase(), W / 2, 97, { align: 'center' });

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.setTextColor(80, 80, 80);
  doc.text(`Designation: ${employee.designation}  |  Department: ${employee.department}`, W / 2, 106, { align: 'center' });
  doc.text(`Project: ${employee.project}`, W / 2, 113, { align: 'center' });

  // Certificate title line
  doc.setFontSize(11);
  doc.setTextColor(60, 60, 60);
  doc.text('has successfully been awarded the', W / 2, 123, { align: 'center' });

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  doc.setTextColor(212, 175, 55);
  doc.text(`"${certTitle}"`, W / 2, 133, { align: 'center' });
  ...
  doc.text('Certificate ID', 40, footerY);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(10, 10, 10);
  doc.text(certId, 40, footerY + 5);

  // Center: Issue date
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(120, 120, 120);
  doc.text('Issue Date', W / 2, footerY, { align: 'center' });
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(10, 10, 10);
  doc.text(new Date(issueDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' }), W / 2, footerY + 5, { align: 'center' });

  // Right: Signature
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(120, 120, 120);
  doc.text('Authorized By', W - 40, footerY, { align: 'right' });
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(10, 10, 10);
  doc.text(issuedBy, W - 40, footerY + 5, { align: 'right' });

  // Signature line
  doc.setDrawColor(10, 10, 10);
  doc.setLineWidth(0.5);
  doc.line(W - 75, footerY + 2, W - 17, footerY + 2);

  // QR code
  if (qrDataUrl) {
    doc.addImage(qrDataUrl, 'PNG', W - 45, footerY - 28, 28, 28);
    doc.setFontSize(6);
    doc.setTextColor(120, 120, 120);
    doc.text('Scan to Verify', W - 31, footerY + 3, { align: 'center' });
  }
  ...
  doc.text('Verify this certificate at kalvixnexus.com/verify', W / 2, H - 15, { align: 'center' });
...
// ─── Certificate Modal ────────────────────────────────────────────────────────
function CertModal({ employee, onClose, onIssued }: {
  employee: Employee;
  onClose: () => void;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Fetch company logo and convert to base64 for PDF embedding
      let logoDataUrl = '';
      try {
      ...
      const pdfDataUrl = await generateCertificatePDF(employee, certId, certTitle, certType, issueDate, issuedBy, qrDataUrl, logoDataUrl);
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-bg-surface border border-gold-primary/10 rounded-lg p-3 text-xs text-text-muted space-y-1">
              ...
              <p className="flex items-center gap-2"><Shield size={12} className="text-gold-primary" /> Certificate ID format: KNX-{new Date().getFullYear()}-XXXXXX</p>
            </div>
          </div>
...
// ─── Employee Form ────────────────────────────────────────────────────────────
function EmployeeForm({ initial, onSave, onCancel, empCount }: {
  initial?: Employee;
  ...
  const [form, setForm] = useState<Omit<Employee, 'id'>>(initial
    ? { ...initial }
    : { ...EMPTY_EMP, employee_id: generateEmployeeId(empCount) }
  );
  ...
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onloadend = () => set_('profile_photo', reader.result as string);
...
// ─── Employee Profile ─────────────────────────────────────────────────────────
function EmployeeProfile({ employee, certificates, onBack, onEdit, onIssueCert, onDelete }: {
  employee: Employee;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-bg-card border border-gold-primary/15 rounded-2xl overflow-hidden">
        <div className="h-24 bg-gradient-to-r from-[#0A0A0A] via-[#1a1505] to-[#0A0A0A] relative">
          <div className="absolute inset-0 opacity-30" style={{ backgroundImage: 'radial-gradient(circle at 30% 50%, rgba(212,175,55,0.4) 0%, transparent 60%)' }} />
        </div>
        <div className="px-6 pb-6">
          <div className="flex items-end gap-4 -mt-10 mb-4">
            <div className="border-4 border-bg-card rounded-full">
            ...
            {empCerts.map(cert => (
              <div key={cert.id} className="bg-bg-surface border border-gold-primary/10 hover:border-gold-primary/25 rounded-xl p-4 flex items-center gap-4 transition-colors group">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {cert.qr_code && <img src={cert.qr_code} alt="QR" className="w-10 h-10 rounded border border-gold-primary/20" />}
                  {cert.pdf_file && (
                    <a
                      href={cert.pdf_file}
                      download={`${cert.certificate_id}.pdf`}
...
// ─── Employee List ────────────────────────────────────────────────────────────
function EmployeeList({ employees, onAdd, onView, onEdit, onDelete, onIssueCert }: {
  employees: Employee[];
  onAdd: () => void;
                  ...
                  <tr key={emp.id} className="hover:bg-bg-surface/50 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-sm font-rajdhani font-bold text-text-primary whitespace-nowrap">{emp.name}</p>
...
export default function EmployeeModule() {
  ...
  const [certModalFor, setCertModalFor] = useState<Employee | null>(null);
    ...
    <div className="relative">
      ...
      {view === 'list' && (
        <EmployeeList
          ...
          onIssueCert={emp => setCertModalFor(emp)}
        />
      )}
      ...
      {view === 'profile' && selectedEmployee && (
        <EmployeeProfile
          ...
          onEdit={() => setView('edit')}
          onIssueCert={() => setCertModalFor(selectedEmployee)}
