...
export default function AboutSection() {
  ...
  const [founderName, setFounderName] = useState("Shourya Sharma");

  useEffect(() => {
    const teamRef = ref(db, 'team/t1');
    const unsubscribe = onValue(teamRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        if (data.image) setFounderImage(data.image);
        if (data.name) setFounderName(data.name);
      }
    });
    return () => unsubscribe();
  }, []);
           ...
           <motion.div
              ...
              >
                <p className="font-semibold text-black">
                  Kalvix Nexus <br /> 
                  <span className="font-normal text-sm">led by {founderName}</span>
                </p>
              </motion.div>
